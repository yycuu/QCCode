...
import { CircleCodeMode, encodeReferencePayload } from "../packages/protocol/src/index.js";
import { privateKeyFromSeed } from "../packages/security/src/index.js";
import { CircleCodeServer } from "../packages/server-sdk/src/index.js";
...
async function server(): Promise<CircleCodeServer> {
  const now = BigInt(Math.floor(Date.now() / 1000));
  return new CircleCodeServer({ issuerId: new Uint8Array(16).fill(4), keyId: 1, privateKeyPkcs8: await privateKeyFromSeed(seed), publicKey, keyNotBefore: now - 100n, keyNotAfter: now + 10_000n });
}
...
describe("reference server replay policy", () => {
  it("allows exactly one concurrent redemption", async () => {
    ...
    const issued = await instance.issue({ mode: CircleCodeMode.REFERENCE, messageType: 4, payload: encodeReferencePayload(1, resourceId), expiresIn: 60, singleUse: true });
  ...
  it("forces CHALLENGE to be single use", async () => {
    ...
    const issued = await instance.issue({ mode: CircleCodeMode.CHALLENGE, messageType: 9, payload, expiresIn: 60 });
