import { encodeCircleCode } from "@circlecode/encoder";
import { fromBase64Url, parseEnvelope, toBase64Url } from "@circlecode/protocol";
import { renderCanvas } from "@circlecode/renderer-canvas";
import { renderSvg } from "@circlecode/renderer-svg";
import { CircleCodeScanner } from "@circlecode/scanner";
import { MemoryTrustStore } from "@circlecode/security";
import "./style.css";

const api = "http://localhost:8787";
const root = document.querySelector<HTMLElement>("#app")!;
root.innerHTML = `
  <header><div class="eyebrow">CIRCLECODE / V1</div><h1>Signed data,<br><em>drawn in circles.</em></h1><p>服务端签名，显示端只编码。每一次验证都保留原始 Envelope。</p></header>
  <section class="workspace">
    <form id="issue"><h2>01 / Server Control</h2><label>Mode<select name="mode"><option>REFERENCE</option><option>CHALLENGE</option><option>INLINE</option></select></label><label>Message Type<input name="messageType" type="number" value="1001"></label><label>Payload<textarea name="payload">{"action":"demo","device":"display-01"}</textarea></label><label>Center Logo URL · optional<input name="logo" value="/circlecode-mark.svg"></label><div class="row"><label>Expires in seconds<input name="expiresIn" type="number" value="300"></label><label class="check"><input name="singleUse" type="checkbox" checked> Single use</label></div><button>Issue signed CircleCode</button></form>
...
let currentEnvelope = "";
let currentLogo = "";
let scannerPromise: Promise<CircleCodeScanner> | undefined;

function getScanner(): Promise<CircleCodeScanner> {
  scannerPromise ??= fetch(`${api}/circlecode/v1/keys`).then(async (response) => {
    ...
    return new CircleCodeScanner(new MemoryTrustStore(records));
  });
  return scannerPromise;
}

async function showScan(scan: Awaited<ReturnType<CircleCodeScanner["scanCanvas"]>>): Promise<void> {
...
async function display(envelopeBase64Url: string): Promise<void> {
  ...
  const symbol = encodeCircleCode(bytes);
  document.querySelector("#symbol")!.innerHTML = renderSvg(symbol, { size: 560, title: "Signed CircleCode", center: currentLogo ? { mode: "logo", imageHref: currentLogo, scale: 0.72 } : { mode: "none" } });
...
document.querySelector("#issue")!.addEventListener("submit", async (event) => {
  ...
  const response = await fetch(`${api}/circlecode/v1/issue`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ mode: data.get("mode"), messageType: Number(data.get("messageType")), expiresIn: Number(data.get("expiresIn")), singleUse: data.get("singleUse") === "on", payload }) });
  const body = await response.json();
...
document.querySelector("#redeem")!.addEventListener("click", async () => {
  const response = await fetch(`${api}/circlecode/v1/redeem`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ envelope: currentEnvelope }) });
  const body = await response.json();
...
const socket = new WebSocket("ws://localhost:8787/circlecode/v1/ws");
socket.addEventListener("open", () => { document.querySelector("#socket")!.textContent = "LIVE"; });
socket.addEventListener("message", (event) => { const message = JSON.parse(String(event.data)); if (message.type === "circlecode.envelope") void display(message.envelope); });
