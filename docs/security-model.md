...
The sparse S1 layout carries a 48-byte bearer envelope without a signature. Its security rests on the 96-bit random resource id, a short lifetime, and atomic server-side single-use redemption; it cannot be verified offline and must never be accepted without a server round trip. C1/C2/C3 formats are deprecated as of v0.3.5 and will be removed in a future release. Migrate to S1 by reissuing signed envelopes as server-backed bearer tokens, not by changing only the layout. Applications that need offline verification must account for its absence in S1; preserve application-specific authorization and user-confirmation checks when moving INLINE/CHALLENGE workflows to server-side resources.
