...
**v0.3.5 弃用提示：** C1/C2/C3 已废弃，将在后续版本移除，建议迁移到 S1。旧格式仍可运行，但 SDK 会在编码、成功解码和服务端旧签名接口中输出英文控制台警告。S1 仅支持 Bearer，签名码须重新签发并改用在线核销，不能直接更换布局或继续依赖离线验签。下文 V1 示例用于维护存量集成；迁移步骤见[生产集成指南](./production-integration.md#v035-弃用与迁移)。
